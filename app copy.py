...
class NewSaleDialog(QDialog):
    ...
    def submit_sale(self):
        ...
        cursor.execute("SELECT id FROM inventory WHERE reg_no = ?", (self.reg_no_search.text(),))
        inventory_record = cursor.fetchone()
        if inventory_record:
            inventory_id = inventory_record[0]
        else:
            QMessageBox.warning(self, "Error", "Inventory item not found.")
            return
        ...
        cursor.execute("""
        INSERT INTO sales (client_name, client_cnic, client_mobile, inventory_id, sale_price, purchase_date, sale_date, payment_method, profit, product_status) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Sold')
        """, (self.client_name.text(), self.client_cnic.text(), self.client_mobile.text(), inventory_id, self.sale_price.text(), self.purchase_date.text(), QDate.currentDate().toString("yyyy-MM-dd"), self.payment_method.text(), self.profit.text()))
    # Rest of the method...

        # Update inventory to mark the product as sold
        cursor.execute("UPDATE inventory SET product_status='Sold' WHERE reg_no = ?", (reg_no,))

        # Insert new user record for the buyer
        cursor.execute("""
            INSERT INTO users (client_name, client_cnic, client_mobile, reg_no, product_status, purchase_date, sale_date, payment_method) 
            VALUES (?, ?, ?, ?, 'Sold', ?, ?, ?)
...
class UserManagementPage(QWidget):
    ...
    def delete_user(self, reg_no):
        ...
        try:
            ...
            conn.close()
        self.load_users()  # Refresh the list after deletion
    # Refresh the list
